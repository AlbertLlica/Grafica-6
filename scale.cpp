...
void escalarCasa(Casa &C, float scale) {
    C.ancho = static_cast<int>(C.ancho * scale);
    C.altura = static_cast<int>(C.altura * scale);
}
...
void trasladarCasa(Casa &C, int dx, int dy) {
    C.x += dx;
    C.y += dy;
}

// Función para trasladar un carro
void trasladarCarro(Carro &C, int dx, int dy) {
    C.x += dx;
    C.y += dy;
}
...
void render() {
    ...
    escalarCasa(casa, 1.5f); // Escalar casa a 1.5 veces su tamaño original
    trasladarCasa(casa, -100, 100);
    crearCasa(casa);

    Carro carro = {400, 100, 200, 50, {0.0f, 0.0f, 1.0f}}; // Color azul
    trasladarCarro(carro, -100, 100); // Trasladar carro a (300, 200)
    crearCarro(carro);
